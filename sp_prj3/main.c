...
int main(void)
{
	...
	char *input, *token, *saveptr, *tmp, *temp;
	...
	while(1){
		...
		temp = (char *)calloc(strlen(saveptr)+1,sizeof(char));
		strcpy(temp,saveptr);
		com_num = is_valid_com(token, temp);
		...
		//if valid command
			...
			switch(com_num){
				...
				case 14 : set_progaddr(saveptr); break;
				case 15 : load_init(); loader_main(saveptr); break;
				case 16 : run_prog(); break;
				case 17 : if(!check_bp(saveptr)) continue; break;
				default : break;
			}
			make_history(tmp);
		}
...
int is_valid_com(char* com, char* nextcom)
{
	...
	if(!strcmp(com,"h") || !strcmp(com,"help"))
		...
		check_num = 13;
	else if(!strcmp(com,"progaddr"))
		check_num = 14;
	else if(!strcmp(com,"loader"))
		check_num = 15;
	else if(!strcmp(com,"run"))
		check_num = 16;
	else if(!strcmp(com,"bp"))
		check_num = 17;
	else
		return 0;
	...
	else if(check_num < 5 || (check_num > 8 && check_num < 11) || check_num == 13 || check_num == 16)
	{
		if(*nextcom){
			token1 = strtok_r(nextcom," \t",&nextcom);
			if(token1)
	...
	else if(check_num == 11 || check_num == 12)
	{
		...
		for(i=0; ;i++){
			...
			if(!token1){
				...
				if(!search_file(token1,check_num-13))
					return 0;
				else{
					if(!(*nextcom))
						break;
	...
	else if(check_num == 14)
	{
		if(*nextcom){
			...
			if(!token1)
				...
				start = hex_to_dec(token1);
				if(start < 0 || start >= ONE_MEGA){
					return 0;
				}
			}
		}
...
//flag=0 : type / flag=1 : assemble / flag=2 : loader
		...
		//if the command is assemble
			...
			if(!strcmp(nextcom, d->d_name)){
				if(flag == 1){
					...
					//if the file ends with .obj
					if(!strcmp(nextcom+strlen(nextcom)-4,".obj")){
						closedir(dp);
						return 1;
					}
				}	
...
int check_bp(char *nextcom)
{
	...
	if(token){
		...
		if(!strcmp(token,"clear"))
			...
			addr = hex_to_dec(token);
			if(addr < 0 || addr >= ONE_MEGA)
				return 0;
			else
				store_bp(addr);
		}
